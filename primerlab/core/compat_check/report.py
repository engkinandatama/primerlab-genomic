...
def generate_markdown_report(result: MultiplexResult, output_dir: Path) -> Path:
    ...
    md = [
        ...
        f"- **Tm Uniformity:** {result.component_scores.get('tm_uniformity', 0):.1f}/100 (Avg Tm: {result.avg_tm:.1f}°C)",
    ...
    md.append("")
    md.append("## 🧩 Primer Pair Details")
    md.append("| Pair | Fwd Tm | Rev Tm | Fwd GC | Rev GC |")
    md.append("| :--- | :--- | :--- | :--- | :--- |")
    
    for pair in result.pairs:
        md.append(f"| {pair.name} | {pair.tm_forward:.1f}°C | {pair.tm_reverse:.1f}°C | {pair.gc_forward:.1f}% | {pair.gc_reverse:.1f}% |")
